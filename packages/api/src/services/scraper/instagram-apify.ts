...
export async function scrapeInstagramBatchViaApify(
  usernames: string[]
): Promise<Map<string, ApifyInstagramData>> {
  const results = new Map<string, ApifyInstagramData>();
  const apify = getClient();
  if (!apify || usernames.length === 0) return results;

  const cleanUsernames = usernames
    .map((u) => u.replace("@", "").trim())
    .filter(Boolean);

  if (cleanUsernames.length === 0) return results;

  try {
    console.log(
      `[Apify] Batch scraping ${cleanUsernames.length} Instagram profiles...`
    );

    const urls = cleanUsernames.map(
    ...
    console.log(
      `[Apify] Batch complete: ${results.size}/${cleanUsernames.length} profiles scraped`
    );
  } catch (error: any) {
    console.error(
      `[Apify] Batch scraping failed:`,
      error.message || error
    );
  }
